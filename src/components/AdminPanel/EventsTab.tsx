...
export const EventsTab: React.FC = () => {
  ...
  const [venue, setVenue] = useState('Ground Floor Seminar Hall, Academic Block-3');
  ...
  const handleOpenAdd = () => {
    ...
    setShortSummary('Hands-on technical sprint on autonomous systems, distributed state, and modern software design.');
    setNarrativeText('Participants engaged in deep architectural challenges, deploying real-time services and competing across multiple rounds.\n\nTeams presented robust prototypes evaluated by industry engineers and faculty coordinators.');
    setGalleryUrls('https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&q=80&w=1200, Opening Keynote\nhttps://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&q=80&w=1200, Sprint Huddle');
    setHighlightsText('50+ Participating Teams\nRs. 15,000 Total Prize Pool\nIndustry Mentors from Top Tech Firms');
    setIsModalOpen(true);
  };
  ...
  const handleDelete = (id: string, eventTitle: string) => {
    if (window.confirm(`Are you sure you want to permanently delete event "${eventTitle}"?`)) {
      playCyberClick();
  ...
  const handleResetDefaults = () => {
    if (window.confirm('Reset all club events back to default CIPHER data?')) {
      playCyberClick();
      resetStoredEvents();
      refreshEvents();
    }
  ...
  const handleSaveSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !shortSummary.trim()) return;
    ...
    const galleryImages: EventGalleryImage[] = galleryUrls
      ...
      .map((line, idx) => {
        ...
        const caption = parts.slice(1).join(',').trim() || `Event Highlight ${idx + 1}`;
        return { url, caption };
      });
    ...
    if (editingEventId) {
      ...
      const updatedItem: EventItem = {
        ...
        fullNarrative,
        galleryImages: galleryImages.length > 0 ? galleryImages : existing?.galleryImages || [],
        highlights,
        tracks: existing?.tracks,
      };
      updateStoredEvent(updatedItem);
    } else {
  ...
  return (
    <div className="space-y-6 font-mono">
      ...
      <div className="rounded-xl border border-emerald-900/90 bg-[#061109]/90 p-4 sm:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-500/90 font-sans mt-0.5">
              Manage hackathons, symposiums, branch galas, and technical workshops displayed on the public site.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            title="Reset events back to default CIPHER curriculum"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {events.map((evt) => {
          const mainImage = evt.galleryImages?.[0]?.url || 'https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&q=80&w=1200';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm">
          <div className="relative max-w-2xl w-full bg-[#060e08] border-2 border-emerald-500/80 rounded-2xl p-6 shadow-[0_0_40px_rgba(16,185,129,0.3)] font-mono text-emerald-100 max-h-[92vh] overflow-y-auto">
            ...
            <div className="border-b border-emerald-900/80 pb-3 mb-4">
              <div className="text-[10px] text-emerald-400 uppercase font-bold tracking-widest">// EVENT ARCHITECT</div>
              <h3 className="text-xl font-bold text-white mt-1">
                {editingEventId ? 'Edit Event Details' : 'Create New CIPHER Event'}
              </h3>
            </div>
            ...
            <form onSubmit={handleSaveSubmit} className="space-y-4 text-xs font-mono">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="sm:col-span-2">
                  <label className="block text-[11px] text-emerald-400 font-bold mb-1">EVENT TITLE *</label>
                  <input
                    type="text"
                    required
                    value={title}
                ...
                <div>
                  <label className="block text-[11px] text-emerald-400 font-bold mb-1">CATEGORY BADGE</label>
                  <input
                    type="text"
                    value={badge}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] text-emerald-400 font-bold mb-1">DISPLAY DATE *</label>
                  <input
                    type="text"
                    required
                    value={dateStr}
                ...
                <div>
                  <label className="block text-[11px] text-emerald-400 font-bold mb-1">ISO DATE (FOR SORTING)</label>
                  <input
                    type="date"
                    value={isoDate}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] text-emerald-400 font-bold mb-1">CAMPUS VENUE *</label>
                  <input
                    type="text"
                    required
                    value={venue}
                ...
                <div>
                  <label className="block text-[11px] text-emerald-400 font-bold mb-1">THEME (OPTIONAL)</label>
                  <input
                    type="text"
                    value={theme}
              ...
              <div>
                <label className="block text-[11px] text-emerald-400 font-bold mb-1">SHORT SUMMARY (CARD PREVIEW) *</label>
                <textarea
                  rows={2}
                  required
                  value={shortSummary}
